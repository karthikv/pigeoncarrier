...
ChromeExOAuth.prototype.sendSignedRequest = function(url, callback,
  ...
  var onProgress = onProgress || false;
 ...
 *     request token.
 */
ChromeExOAuth.prototype.onRequestToken = function(callback, url_callback, xhr) {
  if (xhr.readyState == 4) {
    if (xhr.status == 200) {
      ...
      url = ChromeExOAuth.addURLParam(url, "oauth_callback", url_callback);
      callback(url);
    } else {
